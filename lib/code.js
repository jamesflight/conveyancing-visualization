...
function init(allRateCards) {
  ...
  feeTypes.forEach(el => {
    ...
    console.log(map); // map = flattenPurchase(el, map);
...
function createDiagram(divId, data) {
  ...
  }, new go.Binding("text", "title")), $(go.TextBlock, {
    row: 2,
    column: 2,
...
function splitRateCardsByCountry(rateCardsSplitByProductAndCondsAndOrig) {
  return R.mapObjIndexed((condGroups, product) => R.mapObjIndexed((originatorGroups, cond) => R.mapObjIndexed((cards, originator) => R.groupBy(card => card[10], cards), originatorGroups), condGroups), rateCardsSplitByProductAndCondsAndOrig);
}
