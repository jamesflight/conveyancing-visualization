...
function init(allRateCards) {
    ...
    feeTypes.forEach((el) => {
      ...
      createTreeStructure(el, allRateCards);
    })
}
...
function createDiagram(divId, data) {
  ...
  $(go.Panel, "Table",
    ...
    $(go.TextBlock, { row: 1, column: 1, font: '15pt sans-serif' }, new go.Binding("text", "name")),
    $(go.TextBlock, { row: 2, column: 1, font: '15pt sans-serif' }, new go.Binding("text", "gross")),
    $(go.TextBlock, { row: 3, column: 1, font: '15pt sans-serif' }, new go.Binding("text", "per_applicant")),
  )
);
