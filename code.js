    ...
    $(go.Diagram, divId,
      {
        ...
        layout: $(go.TreeLayout,
                  { angle: 0, layerSpacing: 70, layerStyle: go.TreeLayout.LayerUniform })
      });
...
function getExtraConditionsString(rateCard) {
  return '' + (rateCard[9] ? rateCard[9] : '_') +
    (rateCard[10] ? rateCard[10] : '_') +
    (rateCard[11] ? rateCard[11] : '_') +
    (rateCard[12] ? rateCard[12] : '_') +
    (rateCard[13] ? rateCard[13] : '_') +
    (rateCard[14] ? rateCard[14] : '_') +
    (rateCard[15] ? rateCard[15] : '_');
}
...
function getMapForExtraConditionsRateCards(rateCards) {
  ...
  return rateCards.length === 1
    && rateCards[0].length === 1
    && getExtraConditionsString(rateCards[0][0]) === '_______'
      ? [createRateCardMap(rateCards[0][0])]
      : [];
}
...
function splitRateCardsByExtraConditions(rateCardsSplitByProduct) {
  return R.mapObjIndexed((cards, product) => R.groupBy((card) => card[9], cards), rateCardsSplitByProduct);
}

function splitRateCardsByOriginator(rateCardsSplitByProductAndConds) {
  return R.mapObjIndexed((condGroups, product) =>
    R.mapObjIndexed((cards, cond) =>
      R.groupBy((card) => card[0], cards), condGroups),
    rateCardsSplitByProductAndConds
  );
}

function splitRateCardsByCountry(rateCardsSplitByProductAndCondsAndOrig) {
  return R.mapObjIndexed((condGroups, product) =>
    R.mapObjIndexed((originatorGroups, cond) =>
      R.mapObjIndexed((cards, originator) => 
        R.groupBy((card) => card[10], cards), originatorGroups
      ), condGroups
    ),
    rateCardsSplitByProductAndCondsAndOrig
  );
}
...
function splitRateCardsByPropertyVal(rateCardsSplitByProductAndCondsAndOrigAndCountry) {
  return R.mapObjIndexed((condGroups, product) =>
    R.mapObjIndexed((originatorGroups, cond) =>
      R.mapObjIndexed((countryGroups, originator) => 
        R.mapObjIndexed((cards, country) =>
          R.groupBy((card) => getPropertyValueString(card), cards), countryGroups),
        originatorGroups
      ), condGroups
    ),
    rateCardsSplitByProductAndCondsAndOrigAndCountry
  );
}
...
function tree_chartmap(tree) {
  ...
  R.mapObjIndexed((productGroup, product) => {
    ...
    R.mapObjIndexed((extraConditionsGroup, extraConditions) => {
      if (extraConditions === "") {
        extraConditions = "All properties unless overiden by another fee";
      } else {
        extraConditions = extraConditions + " only";
      }
      map.push({
        parent: product,
